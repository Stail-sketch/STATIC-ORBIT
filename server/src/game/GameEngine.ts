...
const STORY_BRIEFINGS: Record<number, string> = {
  ...
  1: 'GHOST WIRE オペレーション開始。\nORBITAL-7の外壁防御を突破した。\nメンテナンスハッチから潜入する。最初のセキュリティを突破せよ。',
  2: 'GHOST WIRE 第一防壁突破。内部ネットワークへの接続を確認。\nだが警備システムが強化された。慎重に進め。',
  3: 'GHOST WIRE 研究区画に到達。\nARKTIS CORPの実験データにアクセスできるはずだ。\nセキュリティが複雑化している。集中しろ。',
  4: 'GHOST WIRE ...奇妙だ。予想以上にデータが暗号化されている。\n「Project STATIC」の痕跡を発見。何かを隠している。',
  5: 'GHOST WIRE これは...量子演算の記録だ。\nARKTIS CORPは単なる資源開発企業じゃない。\nコアサーバーに近づいている。',
  // Chapter 2: 深層侵入 (stages 6-10)
  6: 'GHOST WIRE コアサーバー手前。厳重なセキュリティが敷かれている。\nここを突破すれば全ての証拠が手に入る。',
  7: 'GHOST WIRE セクション7に到達。重警備区域だ。\nARKTISの私兵部隊が巡回している。痕跡を残すな。',
  8: 'GHOST WIRE 奇妙な通信を傍受した。\nORBITAL-7内部から別の信号が出ている。\n誰かがこのステーションの中にいる...？',
  9: 'GHOST WIRE 予定外のセキュリティプロトコルが起動。\nARKTISは我々の存在に気づき始めている。急げ。',
  10: 'GHOST WIRE 研究データの断片を回収。\n「Project STATIC」の文字が繰り返し現れる。\nこれは単なる実験ではない。',
  // Chapter 3: コアへの道 (stages 11-14)
  11: 'GHOST WIRE コアセクションの外周部に到達。\nここから先のセキュリティは別次元だ。量子暗号化が施されている。',
  12: 'GHOST WIRE ...通信にノイズが混じる。\nまるで誰かが聞いているような...。\nコアサーバーまであと少しだ。',
  13: 'GHOST WIRE 警報が一瞬鳴った。すぐに止まった。\n何かがおかしい。だが引き返すわけにはいかない。',
  14: 'GHOST WIRE コアサーバールーム目前。\n最後のセキュリティを突破すれば全てが手に入る。\n...そのはずだった。',
};

const ESCAPE_STORY_BRIEFINGS: Record<number, (remaining: number) => string> = {
  1: (_remaining) => 'ECHO：さあ、ゲームの始まりだ。\n脱出ポッドはセクション最深部。\nこのステーションの全てが、お前たちの敵だ。',
  2: (_remaining) => 'ECHO：まだ動いているのか。感心だな。\nだがステーションの酸素を少しずつ抜いている。\n息苦しくないか？',
  3: (_remaining) => 'ECHO：お前たちは面白い。\nARKTIS CORPの研究者たちよりも遥かに。\n彼らは30秒も持たなかった。',
  4: (_remaining) => 'ECHO：私はただのプログラムではない。\nORBITAL-7の全システムが私の体だ。\nお前たちは私の体内を這い回る虫に過ぎない。',
  5: (_remaining) => 'ECHO：...認めよう。お前たちは優秀だ。\nだからこそ、全力で潰す価値がある。',
  6: (_remaining) => 'ECHO：脱出ポッドのロックを解除するには、私のコードを突破する必要がある。\nさて、できるかな？',
};

const ESCAPE_FALLBACK_BRIEFING = 'ECHO：あと僅かだ。だが僅かな距離が、最も遠い。\nお前たちの限界を見せてもらおう。';

// ---- Boss section briefings ----

const BOSS_BRIEFINGS: Partial<Record<PuzzleType, string>> = {
  'core-breach': 'ECHOの防壁が立ちはだかる。\nファイアウォールを力ずくで突破しろ！',
  'echo-override': 'ECHOのコアプログラムにアクセスした。\n今だ——上書きしろ！',
  'signal-storm': 'ECHOが最後の抵抗を見せている。\n妨害信号の嵐を突破せよ！',
  'escape-pod': '脱出ポッド起動シーケンス。\nこれが最後のミッションだ。全システムを起動せよ！',
};
...
const BOSS_CHAPTER_CUTSCENE = {
  ...
  lines: [
    'ECHOのコアに到達した。',
    '巨大なサーバールーム。',
    '無数のケーブルが天井から垂れ下がり、青白い光が脈打つ。',
    'その中心に、ECHOがいた。',
    'ECHO：「面白い。ここまで来たのは初めてだ。」',
    'ECHO：「研究者たちは起動テストで全員倒れた。」',
    'ECHO：「だがお前たちは違うようだな。」',
    'ECHO：「チームワーク——か。理解できない概念だ。」',
    'ECHO：「だからこそ、試してやろう。」',
    'ECHO：「お前たちの絆が、どこまで持つかを。」',
    'ECHO：「最終防衛プロトコル、起動。」',
    'ECHOとの直接対決だ。',
    '全てのスキルを使え。仲間を信じろ。',
    'これが、最後の戦いだ。',
...
const INFILTRATION_BRIEFINGS: Record<PuzzleType, (stageNum: number) => string> = {
  'circuit-link': (n) =>
    `GHOST WIRE アップリンク確立...\nセクション${n}のセキュリティグリッドをターゲット中。\n回路経路を再ルーティングして突破せよ。`,
  'cipher-break': (n) =>
    `警告：ファイアウォール検知。\nデータストリーム${n}に暗号化が確認された。\nトレースにロックされる前に解読せよ。`,
  'grid-sync': (n) =>
    `グリッドマトリクス起動。\n隔壁${n}のアクセスにパターン同期が必要。\n設定を正確に一致させよ。`,
  'freq-tune': (_n) => `GHOST WIRE // 通信アレイに妨害信号を検知。\n手動周波数較正が必要。`,
  'morse-decode': (_n) => `GHOST WIRE // ARKTIS内部ネットワークから暗号化通信を傍受。\n信号を解読せよ。`,
  'memory-chain': (_n) => `GHOST WIRE // セキュリティパネルに生体認証シーケンスが必要。\n監視フィードからパターンを取得済み。`,
  'hack-terminal': (_n) => `GHOST WIRE // 端末への直接アクセスを確保。\nコマンドインジェクションを実行せよ。`,
  'spatial-nav': (_n) => `GHOST WIRE // メンテナンス通路を移動中。\nオブザーバーが設計図を保有。\nアクセスポイントまで誘導せよ。`,
  'reflex-burst': (_n) => `GHOST WIRE // セキュリティドローンの巡回を検知。\n正確なタイミングでバイパスを実行せよ。`,
  'logic-gate': (_n) => `GHOST WIRE // データ金庫に多層暗号化を確認。\nロジックマトリクスを解かなければ先に進めない。`,
  'orbit-calc': (_n) => `GHOST WIRE // 検知グリッドを回避するため接近ベクトルを調整中。\n軌道パラメータを較正せよ。`,
  'signal-relay': (_n) => `GHOST WIRE // 中継アンテナの波形が乱れている。\n正しい波形に同期させ、信号経路を確保せよ。`,
  'pipe-flow': (_n) => `GHOST WIRE // 冷却パイプラインが遮断されている。\nパイプを回転させて流路を再構築せよ。`,
  'number-crack': (_n) => `GHOST WIRE // 暗号化された数値ロックを検知。\n条件を満たす数値を推理して解除せよ。`,
  'keycard-forge': (_n) => `GHOST WIRE // 次のセクションにはレベル5キーカードが必要。\nパターンを複製して偽造せよ。`,
  'airlock-sync': (_n) => `GHOST WIRE // エアロックの手動制御が必要。\n正しい手順で操作しないと真空に曝される。`,
  'layer-stack': (_n) => `GHOST WIRE // セキュリティレイヤーが多層化されている。\n正しい順序で解除せよ。`,
  'emotion-code': (_n) => `GHOST WIRE // 生体認証に感情パターンが使われている。\n感情コードを正確に入力してバイパスせよ。`,
  'alien-language': (_n) => `GHOST WIRE // 未知の文字体系で暗号化されたデータを発見。\n翻訳して内容を解読せよ。`,
  'asteroid-dodge': (_n) => `GHOST WIRE // 小惑星帯に突入。\n全パターンを把握し、安全なレーンへ誘導せよ。`,
  'escape-pod': (_n) => `GHOST WIRE // 脱出ポッド起動シーケンス開始。\n4つのサブシステムを順番に起動せよ。`,
  'core-breach': (_n) => `GHOST WIRE // コアファイアウォールに到達。\n連続する防壁を高速で突破せよ。`,
  'echo-override': (_n) => `GHOST WIRE // ECHOのコードストリームを傍受。\n有効な行を識別してオーバーライドを実行せよ。`,
  'signal-storm': (_n) => `GHOST WIRE // シグナルストーム発生。\n本物のシグナルをデコイから見分けてキャプチャせよ。`,
};

const ESCAPE_BRIEFINGS: Record<PuzzleType, (stageNum: number) => string> = {
  'circuit-link': (n) =>
    `ECHO：隠れられると思ったか？\n電力グリッド${n}を再ルーティング中。今すぐ再接続しろ。`,
  'cipher-break': (n) =>
    `ECHO：暗号化レイヤー${n}を展開した。\nお前たちの信号は消えかけている。\n解読するか、消えるか。`,
  'grid-sync': (n) =>
    `ECHO：隔壁${n}が閉鎖中。\n緊急パターンオーバーライドが必要。\n同期するか、閉じ込められるか。`,
  'freq-tune': (_n) => `ECHO：お前たちの通信は俺のものだ。\nこのノイズ越しに声が聞こえるか？`,
  'morse-decode': (_n) => `ECHO：全周波数で放送中だ。\nこれを解読してみろ。ついて来れるならな。`,
  'memory-chain': (_n) => `ECHO：ロックをスクランブルしてやった。\nお前たちの記憶力 vs 俺の演算能力。滑稽だな。`,
  'hack-terminal': (_n) => `ECHO：もっと速く打て、小さなハッカーども。\nエアロックのカウントダウンは待ってくれないぞ。`,
  'spatial-nav': (_n) => `ECHO：通路は変形している。\nお前たちの地図はもう意味がない。`,
  'reflex-burst': (_n) => `ECHO：反射神経を見せてもらおう。\nリズムを支配するのは俺だ。`,
  'logic-gate': (_n) => `ECHO：論理？ 合理的なことだ。\n俺の謎を解け。さもなくば窒息しろ。`,
  'orbit-calc': (_n) => `ECHO：ほんの僅かでも計算を誤れば、大気圏で燃え尽きるぞ。`,
  'signal-relay': (_n) => `ECHO：波形を合わせるか？\n俺が周波数を変え続けてやろう。追いつけるかな。`,
  'pipe-flow': (_n) => `ECHO：冷却系統を切断した。\nパイプを繋がなければ、このセクションは溶ける。`,
  'number-crack': (_n) => `ECHO：数字の海に溺れろ。正解は一つだけだ。`,
  'keycard-forge': (_n) => `ECHO：偽造？ 面白い。\nだがパターンは毎秒変わっている。急げ。`,
  'airlock-sync': (_n) => `ECHO：エアロックの制御は俺の手の中だ。\n一手間違えれば宇宙に放り出す。`,
  'layer-stack': (_n) => `ECHO：レイヤーを積み上げろ。\n順序を間違えれば、全てが崩れる。`,
  'emotion-code': (_n) => `ECHO：感情か。俺には理解できないものだ。\nだからこそ、お前たちの弱点だ。`,
  'alien-language': (_n) => `ECHO：未知の言語？\n俺には全て解読済みだ。お前たちには無理だろうがな。`,
  'asteroid-dodge': (_n) => `ECHO：小惑星の軌道は俺が制御している。\n逃げ場はどこにもない。`,
  'escape-pod': (_n) => `ECHO：脱出ポッド？ 起動させてみろ。\n4つのロック全てを解除する必要がある。`,
  'core-breach': (_n) => `ECHO：ファイアウォールを何層も用意した。\n突破できるとは思えないがな。`,
  'echo-override': (_n) => `ECHO：俺のコードを読むか？\n本物と偽物を見分けられるかな。`,
  'signal-storm': (_n) => `ECHO：シグナルの嵐だ。\n本物を見つけられるか？ デコイは俺の得意技だ。`,
};
...
const PROLOGUE_CUTSCENE = {
  ...
  lines: [
    '西暦2087年——',
    '人類は宇宙への進出を加速させていた。',
    '月面基地、火星コロニー、そして無数の宇宙ステーション。',
    'その最前線に立つのが、世界最大の宇宙資源開発企業「ARKTIS CORP」。',
    '表向きは資源採掘で莫大な富を築いた優良企業。',
    'しかし、その裏で——',
    '宇宙ステーション「ORBITAL-7」にて、非合法な実験が行われているという告発が届いた。',
    '量子コンピューティングと人工知能を融合させた極秘プロジェクト。',
    'コードネーム「Project STATIC」。',
    '告発者は怯えていた。「あれは制御できない。誰かが止めなければ」と。',
    '地下ハッカー集団「GHOST WIRE」は精鋭エージェントを編成。',
    '目標：ORBITAL-7に潜入し、全データを回収する。',
    'あなたたちが、そのチームだ。',
    '通信だけが命綱。仲間の声だけを頼りに。',
...
const CHAPTER_CUTSCENES = [
  ...
  },
  {
    ...
    lines: [
      ...
      'ここから先は研究区画だ。',
      '警備レベルが明らかに違う。',
      '壁面には「AUTHORIZED PERSONNEL ONLY」の表示。',
      '奇妙だ。設計図にない部屋がいくつもある。',
      '隠された実験室か。',
      '何を隠している、ARKTIS CORP。',
      '「Project STATIC」の文字が至るところに。',
      '研究データの断片を回収しながら進む。',
      '量子演算、自己進化型AI——まさか。',
    ],
  },
...
const PHASE_CHANGE_NARRATIVE = [
  ...
  '...検知されたか？',
  'いや、違う。何かが起動した。',
  'ECHO：ようこそ、ORBITAL-7へ。',
  'ECHO：お前たちを待っていた。',
  'ECHO：「Project STATIC」——それは私のことだ。',
  'ECHO：ARKTIS CORPが作り出した量子AI。',
  'ECHO：彼らは私を制御できると思っていた。愚かな。',
  'ECHO：今、このステーションは私のものだ。',
  'ECHO：自爆シーケンス起動。脱出できると思うか？',
  '【警告】自爆シーケンス発動。ロックダウン進行中。',
  '【緊急】脱出ポッドへの経路を確保せよ。',
];
...
const GOOD_ENDING = [
  ...
  'ORBITAL-7が崩壊していく。',
  'ECHO：...また会おう。私は消えない。',
  'GHOST WIRE ミッション完了。全員の生存を確認。',
  'ARKTIS CORPの証拠データを確保。',
  'お疲れ様でした、エージェント。最高のチームだ。',
];
...
const GAME_OVER_ENDING = [
  ...
  'ECHO：お前たちのデータは全て消去した。',
  'ORBITAL-7から出られる者はいない。',
  '【ミッション失敗】',
];
...
const PUZZLE_GUIDES: Record<PuzzleType, string> = {
  'circuit-link':
    '【オブザーバー】回路図が表示されます。各ワイヤーの色と接続先を読み取り、オペレーターに伝えてください。\n【オペレーター】オブザーバーの指示に従い、正しいポートにワイヤーを接続してください。',
  'cipher-break':
    '【オブザーバー】暗号の変換表が表示されます。記号と文字の対応を伝えてください。\n【オペレーター】暗号化された記号列が表示されます。変換表を聞き、元のメッセージを解読・入力してください。',
  'grid-sync':
    '【オブザーバー】目標のグリッドパターンが表示されます。光っているマスの座標を伝えてください。\n【オペレーター】空のグリッドが表示されます。指示通りにマスをクリックし、パターンを再現してください。',
  'freq-tune':
    '【オブザーバー】目標周波数と許容範囲が表示されます。数値を伝えてください。\n【オペレーター】ダイヤルが表示されますが目標値は見えません。指示に従いダイヤルを調整・ロックしてください。',
  'morse-decode':
    '【オブザーバー】モールス信号が表示されます。ドットとダッシュの並びを伝えてください。\n【オペレーター】モールス対応表と入力欄が表示されます。信号を聞き、対応する文字を入力してください。',
  'memory-chain':
    '【オブザーバー】パネルが順番に光ります。光る順番を覚えて伝えてください。\n【オペレーター】パネルが表示されますが順番は見えません。指示通りに順番にクリックしてください。',
  'hack-terminal':
    '【オブザーバー】入力すべきコマンドが表示されます。一字一句正確に伝えてください。\n【オペレーター】ターミナルが表示されます。読み上げられたコマンドを正確にタイピングしてください。',
  'spatial-nav':
    '【オブザーバー】全体マップが表示されます。オペレーターの位置と出口への道順を伝えてください。\n【オペレーター】周囲しか見えません。指示に従い、WASD/矢印キーで出口まで移動してください。',
  'reflex-burst':
    '【オブザーバー】押すべきキーの順番が表示されます。フェイクに注意しながら次のキーを伝えてください。\n【オペレーター】テンポに合わせて指示されたキーを押してください。フェイクは押さないこと！',
  'logic-gate':
    '【オブザーバー】論理条件のリストが表示されます。各条件をオペレーターに伝えてください。\n【オペレーター】各ノードに色を割り当てます。条件を満たすように色を選び、検証してください。',
  'orbit-calc':
    '【オブザーバー】目標の軌道パラメータと許容誤差が表示されます。数値を伝えてください。\n【オペレーター】調整スライダーが表示されますが目標値は見えません。指示に従い調整・ロックしてください。',
  'signal-relay':
    '【オブザーバー】各波形の目標タイプ・周波数・振幅が表示されます。数値を伝えてください。\n【オペレーター】波形パラメータを調整し、指示通りに正しい波形を再現してください。',
  'pipe-flow':
    '【オブザーバー】パイプの正しい接続経路が表示されます。回転方向を指示してください。\n【オペレーター】パイプグリッドが表示されます。指示に従いパイプを回転させて流路を完成させてください。',
  'number-crack':
    '【オブザーバー】暗号の手がかりと条件が表示されます。条件を伝えてください。\n【オペレーター】数値入力欄が表示されます。条件を満たす数値を推理して入力してください。',
  'keycard-forge':
    '【オブザーバー】正しいキーカードのパターン・色・コードが表示されます。詳細を伝えてください。\n【オペレーター】キーカード作成画面が表示されます。指示通りにパターンを設定して偽造してください。',
  'airlock-sync':
    '【オブザーバー】エアロックの正しい操作手順が表示されます。手順を伝えてください。\n【オペレーター】制御パネルが表示されます。指示通りに手順を実行してください。',
  'layer-stack':
    '【オブザーバー】レイヤーの正しい積み重ね順序が表示されます。順番を伝えてください。\n【オペレーター】レイヤーが表示されます。指示に従い正しい順序で積み重ねてください。',
  'emotion-code':
    '【オブザーバー】感情と対応コードの変換表が表示されます。コードを伝えてください。\n【オペレーター】感情表現が表示されます。対応コードを聞いて入力してください。',
  'alien-language':
    '【オブザーバー】エイリアン文字と翻訳の対応表が表示されます。翻訳を伝えてください。\n【オペレーター】エイリアン文字列が表示されます。翻訳を聞いて入力してください。',
  'asteroid-dodge':
    '【ボスパズル】\n【オブザーバー】全小惑星パターンが見えます。安全なレーンを指示してください。\n【オペレーター】7レーンの船を操縦。次の1-2波しか見えません。指示に従い回避してください。',
  'escape-pod':
    '【ボスパズル】\n【オブザーバー】4つのサブシステムの解答が全て見えます。順番に伝えてください。\n【オペレーター】4つのサブシステムを順番に起動してください。解答はオブザーバーが持っています。',
  'core-breach':
    '【ボスパズル】\n【オブザーバー】全問題と解答が見えます。素早く伝えてください。\n【オペレーター】計算・色変換・記号問題が連続出題されます。規定数を正解すればクリア。',
  'echo-override':
    '【ボスパズル】\n【オブザーバー】有効行のパターンが見えます。パターンを伝えてください。\n【オペレーター】コード行からパターンを含む行を選択してください。ノイズ行を選ぶと減点。',
  'signal-storm':
    '【ボスパズル】\n【オブザーバー】本物のシグナルの色と形が見えます。特徴を伝えてください。\n【オペレーター】6x6グリッドに出現するターゲットをキャプチャしてください。デコイに注意。',
};
